...
This document prepares the *PowerWash Simulator* (PWS) session data and fits
the two exploratory behavioural models reported in the manuscript's Study B.
It is its own `targets` stage (`studyb_report`), independent of the Study A
pipeline (`preprocessing.qmd` -> `imputation.qmd` -> `modelling.qmd`) -- it
shares no data with them. `manuscript.qmd` reads this document's exported
objects via `targets::tar_load()` and renders the Study B figure and prose.
...
value of the outcome. Pooling/prediction and inline reporting happen in
`manuscript.qmd`.
