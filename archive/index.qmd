...
if (!file.exists(zip_path)) {
  ...
  # Set longer timeout for large file (194MB)
...
surveys <- surveys |>
  ...
  select(
    ...
    # Life satisfaction (not imputed; used for positive control only)
    life_sat,
...
Results support neither H2a or H2b: at the day level, experiencing neither 1 point higher game need satisfaction (B = `r h2a_day_est`) nor 1 point higher global need frustration (B = `r h2b_day_est`) than usual on a 7-point scale showed meaningful associations with the likelihood of subsequent play (see @fig-h2-predictions). 
...
- use subsequent 6 hours (game NS: B = 0.04; global NF: B = -0.01) or 12 hours (game NS: B = 0.04; global NF: B = -0.02) as the outcome, instead of 24 hours (Sensitivity Analysis 1)
- use alternative random slopes specifications (game NS: B = 0.04; global NF: B = -0.06) (Sensitivity Analysis 2)
- use a count model of minutes played in the subsequent 24 hours instead of binary play/no-play (game NS: B = 0.04; global NF: B = -0.08) (Sensitivity Analysis 3)
- use complete case data instead of imputed data (game NS: B = 0.04; global NF: B = -0.08) (Sensitivity Analysis 4)
- impute data from the full eligible sample instead of the analytic sample (game NS: B = 0.04; global NF: B = -0.08) (Sensitivity Analysis 5)
- include an interaction between gaming need satisfaction and global need frustration (game NS: B = 0.04; global NF: B = -0.08; game NS * global NF: B = 0.04) (Sensitivity Analysis 6)
...
The 30-day aggregate associations (game need satisfaction: B = `r h2a_agg_est`; global need frustration: B = `r h2b_agg_est`) similarly indicate no relationship across individuals; these are reported as exploratory and unrelated to the focal hypotheses about short-term behavioral dynamics. 
...
Results are shown in @fig-study2-results. At the session level, game need satisfaction showed a very weak association with session length (B = `r pws_m1_est`). Panel A illustrates that when players experienced higher need satisfaction than their personal average, they did not play meaningfully longer: if a player who typically reported need satisfaction of 1 instead reported a 7, their expected change in session length would be only `r pws_m1_max_eff`.
...
Broadly, results therefore converge with Study A: Despite examining `r pws_n_sessions_fmt` sessions from `r pws_n_players_fmt` players in a different game using different measures, the pattern of weak behavioral associations replicated. Full model outputs are provided in the Appendix (@tbl-full-study2).
