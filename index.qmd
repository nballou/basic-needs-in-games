...
# Reconstruct the analytical-sample wide table and MICE where-matrix (same
# functions the imputation stage uses) purely to report missingness rates.
sw_ms <- build_wide_survey(surveys, min_waves = 15)
mice_setup_ms <- setup_mice_inputs(sw_ms)
vars_imp_ms <- names(sw_ms)[mice_setup_ms$methods != ""]

overall_miss_pct <- scales::number(100 * mean(mice_setup_ms$where), 0.1)
pct_ppl_with_missing <- scales::number(
  100 * mean(rowSums(is.na(sw_ms[vars_imp_ms])) > 0),
  0.1
)
```
...
Following the two-stage protocol from @VonHippel2020How based on fraction of missing information, we used 27 imputations. For all variables, imputed distributions closely overlapped with the observed data (see the Supplement). Models were fit separately to each imputed dataset, and estimates were pooled across imputations using Rubin's rules as implemented in the `mice` package [@vanBuurenGroothuis-Oudshoorn2011mice]. For comparison, complete case analyses are reported in the Appendix (Sensitivity Analysis 4); results do not meaningfully differ from the imputed analyses.
...
# S4: complete cases and full eligible sample (logistic), from the combined
...
s4_full_gamens <- sample_or("Full sample (preregistered)", "game_ns_cw")
s4_full_globalnf <- sample_or("Full sample (preregistered)", "global_nf_cw")
...
Because the outcome is binary, estimates are reported as odds ratios (OR), where 1 indicates no association. Results support neither H2a nor H2b: at the day level, a 1-point higher than usual score on the 7-point scale in game need satisfaction (OR = `r h2a_day_est`) or global need frustration (OR = `r h2b_day_est`) was not meaningfully associated with the odds of subsequent play (see @fig-h2-predictions).

We conducted a wide range of sensitivity analyses providing convergent conclusions. key estimates for gaming need satisfaction (game NS) and global need frustration (global NF) remain indistinguishable from the null in models that:
...
- use complete case data instead of imputed data (game NS: OR = `r s4_cc_gamens`; global NF: OR = `r s4_cc_globalnf`) (Sensitivity Analysis 4)
- impute data from the full eligible sample instead of the analytic sample (game NS: OR = `r s4_full_gamens`; global NF: OR = `r s4_full_globalnf`) (Sensitivity Analysis 5)
- include an interaction between gaming need satisfaction and global need frustration (game NS: OR = `r s5_int_gamens`; global NF: OR = `r s5_int_globalnf`; game NS × global NF: ratio of ORs = `r s5_int_product`) (Sensitivity Analysis 6)
...
bind_rows(h1_combined, h2_combined, h3_combined) |>
  ...
  tt(
    notes = "Estimate is a raw coefficient. H2 has a binary outcome (logistic model); the OR column gives the odds ratio and its 95% CI. H1 and H3 are linear."
  ) |>
  style_tt(fontsize = 0.7) |>
  style_tt(i = 0, bold = TRUE)
```
...
#| tbl-cap: "S4: H2 estimates (odds ratios) across sample-construction and imputation specifications."
...
#| fig-cap: "S4: H2 estimates (odds ratios) across sample-construction and imputation specifications. Points show pooled odds ratios; bars show 95% confidence intervals (log scale). The primary specification (threshold = 15 waves) is highlighted."
