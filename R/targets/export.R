...
# imputation.qmd: MICE + within/between-person centering. dat_observed is the
# .imp == 0 (non-imputed) slice of the long frame, for the observed-vs-imputed
# diagnostic and the complete-case sensitivity analyses.
...
# modelling.qmd: H1/H2/H3 confirmatory model fitting.
...
sensitivity_export_names <- c(
  ...
  "s2_bayes_fixed", # S2: Bayesian random-slopes fixed effects
  "s2_h2a_pooled", # S2: random slope for game NS only
  "s2_h2b_pooled", # S2: random slope for global NF only
